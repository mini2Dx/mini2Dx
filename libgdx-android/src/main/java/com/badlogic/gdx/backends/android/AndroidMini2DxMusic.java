 ...
 *
 */
public class AndroidMini2DxMusic implements Music, MediaPlayer.OnCompletionListener, org.mini2Dx.core.audio.Music {
	private final AndroidMini2DxAudio audio;
	...
	AndroidMini2DxMusic (AndroidMini2DxAudio audio, MediaPlayer player) {
		this.audio = audio;
		this.player = player;
	...
	public void dispose () {
		...
		try {
			...
			Gdx.app.log("AndroidMini2DxMusic", "error while disposing AndroidMini2DxMusic instance, non-fatal");
		} finally {
			player = null;
	...
	public void onCompletion (MediaPlayer mp) {
		if (onCompletionListener != null) {
			Gdx.app.postRunnable(new Runnable() {
				@Override
				public void run () {
					onCompletionListener.onCompletion(AndroidMini2DxMusic.this);
				}
			});
		}
	};
