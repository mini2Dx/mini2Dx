...
import org.mini2Dx.core.input.nswitch.SwitchDualJoyConGamePadAdapter;
...
/**
 * {@link SwitchDualJoyConGamePad} implementation of {@link GamePadUiInput}
 */
public class SwitchDualJoyConUiInput extends SwitchDualJoyConGamePadAdapter implements GamePadUiInput<SwitchDualJoyConButton> {
	private static final String ID_PREFIX = SwitchDualJoyConUiInput.class.getSimpleName() + "-";
	...
	private SwitchDualJoyConButton actionButton = SwitchDualJoyConButton.A;
	...
	public SwitchDualJoyConUiInput(UiContainer uiContainer) {
		super();
	...
	@Override
	public boolean leftStickXMoved(SwitchDualJoyConGamePad controller, float value) {
		if(!enabled) {
			return false;
		}
		if(value < -stickThreshold) {
			uiContainer.setLastInputSource(InputSource.CONTROLLER);
			uiContainer.setLastGamePadType(GamePadType.SWITCH_DUAL_JOYCON);
			left = true;
			right = false;
	...
	@Override
	public boolean leftStickYMoved(SwitchDualJoyConGamePad controller, float value) {
		if(!enabled) {
			return false;
		}
		if(value < -stickThreshold) {
	...
	@Override
	public boolean buttonDown(SwitchDualJoyConGamePad controller, SwitchDualJoyConButton button) {
		if(!enabled) {
			return false;
		}
		uiContainer.setLastInputSource(InputSource.CONTROLLER);
		...
		if(navigateWithDPad) {
			switch (button) {
			case UP:
				return uiContainer.keyDownNoInputChange(Input.Keys.UP);
			case DOWN:
				return uiContainer.keyDownNoInputChange(Input.Keys.DOWN);
			case LEFT:
				return uiContainer.keyDownNoInputChange(Input.Keys.LEFT);
			case RIGHT:
				return uiContainer.keyDownNoInputChange(Input.Keys.RIGHT);
			}
		}
	...
	@Override
	public boolean buttonUp(SwitchDualJoyConGamePad controller, SwitchDualJoyConButton button) {
		if(!enabled) {
			return false;
		}
		uiContainer.setLastInputSource(InputSource.CONTROLLER);
		...
		if(navigateWithDPad) {
			switch (button) {
			case UP:
				return uiContainer.keyUp(Input.Keys.UP);
			case DOWN:
				return uiContainer.keyUp(Input.Keys.DOWN);
			case LEFT:
				return uiContainer.keyUp(Input.Keys.LEFT);
			case RIGHT:
				return uiContainer.keyUp(Input.Keys.RIGHT);
			}
		}
	...
	@Override
	public SwitchDualJoyConButton getActionButton() {
		return actionButton;
	}

	@Override
	public void setActionButton(SwitchDualJoyConButton actionButton) {
	...
	public boolean equals(Object obj) {
		...
		if (getClass() != obj.getClass())
			return false;
		SwitchDualJoyConUiInput other = (SwitchDualJoyConUiInput) obj;
		if (id == null) {
			if (other.id != null)
				return false;
