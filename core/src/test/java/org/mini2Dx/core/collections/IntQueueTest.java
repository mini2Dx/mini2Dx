...
public class IntQueueTest {
    ...
    public void resizableQueueTest () {
        final IntQueue q = new IntQueue(8);
    ...
    @Test
    public void getTest () {
        final IntQueue q = new IntQueue(7);
        for (int i = 0; i < 5; i++) {
            for (int j = 0; j < 4; j++) {
                q.addLast(j);
            }
    ...
    private void assertEqualsAndHash (IntQueue q1, IntQueue q2) {
        assertEquals(q1, q2);
        assertEquals("Hash codes are not equal", q1.hashCode(), q2.hashCode());
    }

    private void assertValues (IntQueue q, int... values) {
