...
public class BooleanQueue {
    ...
    private BooleanQueueIterable iterable;
    ...
    public Iterator<Boolean> iterator () {
        if (iterable == null) iterable = new BooleanQueueIterable(this);
    ...
    public boolean equals (Object o) {
        ...
        if (!(o instanceof BooleanQueue)) return false;

        BooleanQueue q = (BooleanQueue)o;
        final int size = this.size;
    ...
    static public class BooleanQueueIterator implements Iterator<Boolean>, Iterable<Boolean> {
        private final BooleanQueue queue;
        private final boolean allowRemove;
        int index;
        boolean valid = true;


        public BooleanQueueIterator (BooleanQueue queue) {
            this(queue, true);
        }

        public BooleanQueueIterator (BooleanQueue queue, boolean allowRemove) {
            this.queue = queue;
    ...
    static public class BooleanQueueIterable implements Iterable<Boolean> {
        private final BooleanQueue queue;
        private final boolean allowRemove;
        private BooleanQueueIterator iterator1, iterator2;

        public BooleanQueueIterable (BooleanQueue queue) {
            this(queue, true);
        }

        public BooleanQueueIterable (BooleanQueue queue, boolean allowRemove) {
            this.queue = queue;
            this.allowRemove = allowRemove;
        }
        ...
        public Iterator<Boolean> iterator () {
            if (iterator1 == null) {
                iterator1 = new BooleanQueueIterator(queue, allowRemove);
                iterator2 = new BooleanQueueIterator(queue, allowRemove);
            }
            if (!iterator1.valid) {
                iterator1.index = 0;
