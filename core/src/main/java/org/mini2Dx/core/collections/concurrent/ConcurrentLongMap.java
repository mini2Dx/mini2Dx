...
public class ConcurrentLongMap<T> extends LongMap<T> implements ConcurrentCollection {
     ...
     * @return True if the value was put
     */
    public boolean putIfPresent(long key, T value) {
        boolean result = false;
        lock.lockWrite();
        if(super.containsKey(key)) {
            super.put(key, value);
    ...
    @Override
    public void putAll(LongMap<? extends  T> map) {
