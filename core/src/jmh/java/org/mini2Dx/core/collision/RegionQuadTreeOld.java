...
public class RegionQuadTreeOld<T extends Sizeable> extends PointQuadTreeOld<T> {
     ...
     */
    public RegionQuadTreeOld(int elementLimit, int mergeWatermark, float x, float y, float width, float height) {
     ...
     *            The height of the {@link RegionQuadTree}
     */
    public RegionQuadTreeOld(int elementLimit, float x, float y, float width, float height) {
     ...
     *            The height of the {@link RegionQuadTree}
     */
    public RegionQuadTreeOld(RegionQuadTreeOld<T> parent, float x, float y, float width, float height) {
     ...
     *            The height of the {@link RegionQuadTree}
     */
    public RegionQuadTreeOld(float minimumQuadWidth, float minimumQuadHeight, int elementLimitPerQuad, int mergeWatermark,
    ...
    public void warmupPool(int poolSize, int expectedElementsPerQuad) {
        ...
        for(int i = 0; i < poolSize; i++) {
            RegionQuadTreeOld<T> regionQuadTree = new RegionQuadTreeOld<T>(this, 0, 0, 1, 1);
    ...
    protected RegionQuadTreeOld<T> allocate(RegionQuadTreeOld<T> parent, float x, float y, float width, float height) {
        if(pool == null || pool.size == 0) {
            return new RegionQuadTreeOld<>(parent, x, y, width, height);
        }
        final RegionQuadTreeOld<T> result = (RegionQuadTreeOld) pool.removeFirst();
    ...
    private void getElementsWithinAreaUpwards(Array<T> result, Shape area, boolean firstInvocation, boolean childNodeCrossed) {
        ...
        ((RegionQuadTreeOld<T>)parent).getElementsWithinAreaUpwards(result, area, false, nodeCrossed);
    }
    ...
    public void getElementsWithinAreaIgnoringEdges(Array<T> result, Shape area) {
        if (topLeft != null) {
            ...
            if (topLeft.isSearchRequired()) {
                ...
                } else if(topLeft.intersectsIgnoringEdges(area)) {
                    topLeft.getElementsWithinAreaIgnoringEdges(result, area);
                }
            }
            ...
            if (!containedInQuad && topRight.isSearchRequired()) {
                if(topRight.contains(area)) {
                    ...
                    containedInQuad = true;
                } else if(topRight.intersectsIgnoringEdges(area)) {
                    topRight.getElementsWithinAreaIgnoringEdges(result, area);
                }
            }
            ...
            if (!containedInQuad && bottomLeft.isSearchRequired()) {
                if(bottomLeft.contains(area)) {
                    ...
                    containedInQuad = true;
                } else if(bottomLeft.intersectsIgnoringEdges(area)) {
                    bottomLeft.getElementsWithinAreaIgnoringEdges(result, area);
                }
            }
            ...
            if (!containedInQuad && bottomRight.isSearchRequired()) {
                if(bottomRight.contains(area)) {
                    ...
                    containedInQuad = true;
                } else if(bottomRight.intersectsIgnoringEdges(area)) {
                    bottomRight.getElementsWithinAreaIgnoringEdges(result, area);
                }
            }
        }
    ...
    private void getElementsWithinAreaIgnoringEdgesUpwards(Array<T> result, Shape area, boolean firstInvocation) {
        ...
        if (parent != null) {
            if (!this.contains(area)) {
                ...
                if (parent.topLeft != this && parent.topLeft.isSearchRequired() && area.intersectsIgnoringEdges(parent.topLeft)) {
                    parent.topLeft.getElementsWithinAreaIgnoringEdges(result, area);
                }
                if (parent.topRight != this && parent.topRight.isSearchRequired() && area.intersectsIgnoringEdges(parent.topRight)) {
                    parent.topRight.getElementsWithinAreaIgnoringEdges(result, area);
                }
                if (parent.bottomLeft != this && parent.bottomLeft.isSearchRequired() && area.intersectsIgnoringEdges(parent.bottomLeft)) {
                    parent.bottomLeft.getElementsWithinAreaIgnoringEdges(result, area);
                }
                if (parent.bottomRight != this && parent.bottomRight.isSearchRequired() && area.intersectsIgnoringEdges(parent.bottomRight)) {
                    parent.bottomRight.getElementsWithinAreaIgnoringEdges(result, area);
                }
            }

            ((RegionQuadTreeOld<T>)parent).getElementsWithinAreaIgnoringEdgesUpwards(result, area, false);
        }
    }
    ...
    protected void getElementsContainingAreaUpwards(Array<T> result, Shape area, boolean firstInvocation, boolean entirelyContained) {
        ...
        if (parent != null) {
            ...
            ((RegionQuadTreeOld<T>)parent).getElementsContainingAreaUpwards(result, area, false, entirelyContained);
        }
    }
    ...
    private void getElementsContainingPointUpwards(Array<T> result, Point point, boolean firstInvocation) {
        ...
        if (parent != null){
            ...
            ((RegionQuadTreeOld<T>)parent).getElementsContainingPointUpwards(result, point, false);
        }
    }
    ...
    private void addElementsIntersectingLineSegmentUpwards(Array<T> result, LineSegment lineSegment, boolean firstInvocation) {
        ...
        if (parent != null) {
            ...
            ((RegionQuadTreeOld<T>)parent).addElementsIntersectingLineSegmentUpwards(result, lineSegment, false);
        }
    }
    ...
    public void getElements(Array<T> result) {
        if (topLeft != null) {
            ((RegionQuadTreeOld<T>) topLeft).getElements(result);
            ((RegionQuadTreeOld<T>) topRight).getElements(result);
            ((RegionQuadTreeOld<T>) bottomLeft).getElements(result);
            ((RegionQuadTreeOld<T>) bottomRight).getElements(result);
        }
        result.addAll(elements);
    }
